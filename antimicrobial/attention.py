...
def pHLA_attns_draw_save(data, attn_data, hla = False, peptide = False, attn_savepath = False, fig_savepath = False):
    print("data=",data)
    idx = data[data.peptide == peptide].index[0]
    # hla_sequence = list(data.iloc[idx].HLA_sequence)
    ...
    temp_pd = pd.DataFrame(np.array(temp), columns = list(peptide))
    ...
    if attn_savepath: 
        temp_pd.to_csv(attn_savepath + '{}_{}_attention.csv'.format(hla.replace('*', '_'), peptide))
