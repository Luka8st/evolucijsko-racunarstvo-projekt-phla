...
def _eachhead_peplength_pepposition(SUM_length_head_dict, label = None, savepath = False):
...
def pHLA_attns_draw_save(data, attn_data, hla = False, peptide = False, attn_savepath = False, fig_savepath = False):
    min_idx = data[data.HLA == "HLA-A*01:01"][data.peptide == "DEFVVVTV"].index[0]
    print("min_idx")


    print("here,", attn_data)
    print("hla, pep:", hla, peptide)
    idx = data[data.HLA == hla][data.peptide == peptide].index[0] - min_idx
    # idx = data[data.HLA == hla][data.peptide == peptide].index[0]
    print("idx in draw_save=", idx)
    print("data:", data)
    ...
    if attn_savepath: 
        print("hla, peptide:", hla, peptide)
        print("path", (attn_savepath + '{}_{}_attention.csv'.format(hla.replace('*', '_').replace(':', '_'), peptide)))
        temp_pd.to_csv(attn_savepath + '{}_{}_attention.csv'.format(hla.replace('*', '_').replace(':', '_'), peptide))
        #temp_pd.to_csv(attn_savepath + 'HLA-A11:01_{}_attention.csv'.format(peptide))
    ...
    if fig_savepath:
        ...
        plt.savefig(fig_savepath + '{}_{}_{}_Prob{}.tif'.format(hla.replace('*', '_'), peptide, ['Negative', 'Positive'][y_pred == 1], y_prob),
                dpi = 600, pil_kwargs = {'compression': 'tiff_lzw'}, bbox_inches = 'tight', facecolor=fig.get_facecolor())
